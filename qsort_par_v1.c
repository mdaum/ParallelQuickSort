...
void quicksort(double * data, int lo, int hi, int n, int seq_threshold){
	if(hi>lo){
		if(hi-lo <seq_threshold){
			...
			quicksort(data,lo,pivot-1,n,seq_threshold);
			#pragma omp task firstprivate(data,lo,hi,pivot)
			quicksort(data,pivot+1,hi,n,seq_threshold);
		}
	}
}
...
int main(int argc,char *argv[]){
	...
	quicksort(data,0,n-1,n,seq_threshold);//do sort
	}
	t2=omp_get_wtime();
